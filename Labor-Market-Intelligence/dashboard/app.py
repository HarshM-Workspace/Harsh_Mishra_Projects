...
def load_all_data():
    ...
    # Fix 2: Live total unique job count (raw observed postings across dataset)
    total_jobs = 0
    for q in [
        "SELECT COUNT(DISTINCT job_id) FROM main_staging.stg_adzuna_postings",
        "SELECT COUNT(DISTINCT job_id) FROM staging.stg_adzuna_postings",
        "SELECT COUNT(DISTINCT job_id) FROM stg_adzuna_postings",
        "SELECT COUNT(DISTINCT job_id) FROM main_staging.stg_adzuna",
        "SELECT COUNT(DISTINCT job_id) FROM staging.stg_adzuna",
        "SELECT COUNT(DISTINCT job_id) FROM stg_adzuna",
    ]:
        try:
    ...
    if total_jobs == 0:
        try:
            csv_path = (Path(__file__).parent.parent / "raw" / "adzuna" / "adzuna_extracted.csv").resolve()
            if csv_path.exists():
                val = duckdb.connect().execute(
                    f"SELECT COUNT(DISTINCT id) FROM read_csv_auto('{csv_path.as_posix()}', header=true)"
                ).fetchone()[0]
                if val and val > 0:
                    total_jobs = int(val)
        except Exception:
